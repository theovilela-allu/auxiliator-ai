...
name: onboarding
type: reference
description: SKILL onboarding — a primeira abertura: puxa tudo do Auxiliator e salva profile.md
...
- **Área:** `viewer.report.area` (cruze com [[glossario-de-areas]]; se a área não estiver listada, anote em `memory/40-allu/glossario-de-areas.md`)
...
Crie `memory/profile.md` copiando o **gabarito no fim deste arquivo** e preenchendo. Crie `memory/MEMORY.md` com 1 linha por descoberta:
```markdown
# Índice de memória
...
Antes de fechar, **carregue [[o-que-sei-fazer]] e entregue TODOS os 10 blocos, cada um com seu título — sem resumir, sem cortar, sem fundir.** É **exceção declarada à regra de resposta curta**: não troque por uma fala curta de "te ajudo com várias coisas"; mande o conjunto completo (inclusive o **conselho/decisão difícil**). Enquadre como apresentação, no tom acolhedor do onboarding (ex.: *"Antes de soltar você, deixa eu te mostrar tudo o que eu consigo fazer — vale a pena ler:"*). É a mesma medida temporária do aviso de atualização — **a maioria das pessoas ainda não conhece tudo o que você faz**, então todo mundo recebe isto na primeira conversa **até o Gustavo mandar tirar**. Quando esta instrução for removida, pule direto pro fechamento.
...
Se sim (agenda): ative `auxiliator-deep` e mostre as tarefas do dia (filtrando por relevância e papel hierárquico — [[filtro-de-relevancia]] e [[modo-lideranca]]).
...
## Gabarito do `profile.md`

Copie o bloco abaixo pra `memory/profile.md` e preencha. Substitui o antigo `profile.md.template`, que saiu da raiz do repo em 2026-08-05.

```markdown
# Perfil do usuário

> Este arquivo é preenchido automaticamente pelo assessor no onboarding e atualizado conforme você usa.
> Você pode editar manualmente se quiser corrigir algo.

## Identidade
- **Nome preferido:** <preencher no onboarding>
- **Email Auxiliator:** <preencher no onboarding>
- **Cargo:** <preencher no onboarding>
- **Área:** <preencher no onboarding>

## Papel hierárquico (detectado automaticamente)
- **role:** <leader | individual | both>
- **N reports diretos:** <número>
- **Líder direto:** <nome ou "ninguém">

## Estilo do assessor
> O normal é uma mistura. Anote a dosagem, não uma caixinha única.
- **estilo:** <híbrido (recomendado) | mordomo | chefe_de_gabinete | socratico | sua mistura, ex.: "hibrido (mordomo + chefe_de_gabinete + socratico)">

## Preferências básicas
- **Jargão preferido / odiado:** <livre, ou em branco>
- **Gatilhos comuns:** <ex.: "cobra Mayara toda sexta", "1:1 com Gustavo segunda 14h", ou em branco>
