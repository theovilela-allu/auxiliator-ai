...
type: reference
description: SKILL auxiliator-deep — o boot de 5 passos da conversa e a porta pra operar o Auxiliator
...
**BOOT** (abriu esta skill no início da conversa, sem pedido específico): faça os 5 passos abaixo e pare. **Não** carregue [[auxiliator-deep-operar]] nem [[api-do-auxiliator]] — no boot você só lê estado.

**OPERAR** (a pessoa pediu pra ver/criar/mover/editar algo): garanta a sessão (passo 1) e carregue **[[auxiliator-deep-operar]]** (nesta pasta) + **[[api-do-auxiliator]]**. Sem o manual você adivinha, e adivinhar quebra.
...
- Caiu na tela de login, ou apareceu aviso de tela cheia de "mudou de endereço", ou o navegador está num endereço antigo → carregue **[[auxiliator-deep-login]]** (nesta pasta) e siga de lá.

### 2. Puxar o panorama do dia

`await Aux.state()` (ou o cache de ≤15min, se existir — regra em [[auxiliator-deep-operar]]). Monte, filtrando em JS dentro do navegador pra trazer só o resumo:
...
- **Se for líder** (`viewer.isLeader`): o que pega fogo no time — vencidas e bloqueadas de quem está em `viewer.visibleIds`. Carregue [[modo-lideranca]] pro tom.
...
4. Se `latest` ≠ sua versão local, **ou** `latest` ≠ versão anunciada → carregue **[[auxiliator-deep-atualizacao]]** (nesta pasta) e siga de lá.
