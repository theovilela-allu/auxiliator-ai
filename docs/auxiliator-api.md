...
| **Cronograma da meta (entregas)** | `await Aux.milestones.listForGoal(goal_id)` · `create({ goal_id, title, due_date?, done? })` · `update(id, patch)` · `remove(id)` — ENTREGAS (marcos) da linha do tempo que aparece no drill-down da meta (aba Tarefas filtrada por ela). `title` obrigatório; `done` é **decisão manual da pessoa** (o progresso x/y das tarefas linkadas é só guia — NUNCA conclua uma entrega por conta própria). `remove()` só deslinka as tarefas, não as apaga. Entregas NÃO vêm em `state()` — leia com `listForGoal` |
| Linkar tarefas a uma entrega | `await Aux.milestones.setTasks(milestone_id, [addIds], [removeIds])` em lote, OU `milestone_id: <id>` direto no `tasks.create`/`update`/`saveWithOwners`. Cada tarefa fica em **no máximo uma** entrega (linkar troca). A tarefa precisa ter `goal_id` DA MESMA meta (senão o servidor anula o vínculo em silêncio) e ser tarefa de card (topo ou filha de Projeto — subtarefa de checklist não conta no progresso da entrega) |
